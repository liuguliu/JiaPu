...
public class People {
    ...
    String daughterinfo="";
    String soninfo="";
    GlobalRule GR = new GlobalRule();


    @Override
    public String toString() {
        // TODO Auto-generated method stub
        return id+","+name+","+fatherid+","+motherid+","+partnerid+","+gender+","+CourtesyName+","+pesudonym+","+cengName+","+
                ChineseBirthday+","+Chinesedeathdate+","+birthday+","+deathdate+","+buried+","+familyrank+","+
                shi+","+generation+","+address+","+occupation+","+family+","+nationality+","+description;
    }
        ...
        if(childrennum<sons.length) {
            for(int j=0;j<childrennum;j++) {
                ...
                for(int k=0;k<TreeNUM;k++) {
                    if((p[k].name.equals(nameson)||p[k].name.equals(GR.generationRank[shi]+nameson))
    ...
    public int getDaughter(int personnum,int imotherid, String ms, String daughtersinfo, People[] p) {
        ...
        if(!daughtersinfo.contains(" ")) {
            ...
            if(childrennum<daughters.length) {
                for(int j=0;j<childrennum;j++) {
                    if(daughters[j].contains(GR.rankNum[j]+"适")||daughters[j].charAt(0)=='适') {
                        p[personnum]=new People();
                        p[personnum].id=personnum+1;
                        p[personnum].name="女"+GR.cNum[j];
                        p[personnum].gender="女";
                        ...
                        p[personnum].fatherid = id;
                        p[personnum].motherid = imotherid;
                        if(daughters[j].contains(GR.rankNum[j]+"适")) {
